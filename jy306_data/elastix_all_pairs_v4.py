...
N_RESOLUTIONS = 4
MAX_ITER = 500
ITER_CHECKPOINTS = [10, 50, 100, 200, 300, 500]
...
def run_elastix_at_iter(ref_ds, mov_ds, n_iter, save_transform_path=None):
    """Run Elastix for n_iter iterations and return warped DS image.
    If save_transform_path is given, saves TransformParameters.0.txt there."""
    ...
    if save_transform_path is not None:
        ...
        for i, t in enumerate(tmap):
            sitk.WriteParameterFile(t, f'{save_transform_path}/TransformParameters.{i}.txt')
...
pairs = [(a, b) for a, b in all_pairs
         ...
         and 'row5' not in a and 'row5' not in b]
print(f"Processing {len(pairs)}/20 pairs (rows 2-4 only)")

needed_keys = set(k for a, b in pairs for k in (a, b))

# Load boundary slices for needed tiles only
...
out_dir = f'{BASE}/png_exports/z_stitch_elastix_v4_500iter'
...
for idx, (key_a, key_b) in enumerate(pairs):
    ...
    for cp in ITER_CHECKPOINTS:
        ...
        try:
            save_tfm = pair_dir if cp == ITER_CHECKPOINTS[-1] else None
            result_ds = run_elastix_at_iter(ref_ds, mov_ds, cp, save_transform_path=save_tfm)
...
# Save results JSON
with open(f'{out_dir}/elastix_results_v4.json', 'w') as f:
    json.dump(all_results, f, indent=2)
print(f"Results saved: {out_dir}/elastix_results_v4.json")
