...
IMG_DIR = f'/Users/neurolab/neuroinformatics/margaret/png_exports/registration_video'

def load_boundary_slices(key, mask):
    first = cv2.imread(f'{IMG_DIR}/{key}/GFP_z000.png', cv2.IMREAD_GRAYSCALE)
    last  = cv2.imread(f'{IMG_DIR}/{key}/GFP_z011.png', cv2.IMREAD_GRAYSCALE)
    if first is None or last is None:
        raise FileNotFoundError(f"Missing PNG for {key}")
    return (first.astype(np.float32) * mask.astype(np.float32),
            last.astype(np.float32)  * mask.astype(np.float32))
...
# ============================================================
# Only reprocess pairs where at least one tile is in rows 2-4
ROWS_CHANGED = {'row2', 'row3', 'row4'}
ROWS_EXCLUDE = {'row5'}  # don't load row5 tiles
...
pairs = [(a, b) for a, b in all_pairs
         if (a.split('_')[0] in ROWS_CHANGED or b.split('_')[0] in ROWS_CHANGED)
         and a.split('_')[0] not in ROWS_EXCLUDE
         and b.split('_')[0] not in ROWS_EXCLUDE]
print(f"Reprocessing {len(pairs)}/20 pairs (rows 2-4 only)\n")

# Load existing transforms and merge (keep row1 and row5 as-is)
transforms = {}
old_path = f'{BASE}/registration_video/auto_align_transforms_iou.json'
if os.path.exists(old_path):
    with open(old_path, 'r') as f:
        transforms = json.load(f)
    print(f"Loaded {len(transforms)} existing transforms from iou.json")

qc_dir = f'{BASE}/png_exports/z_stitch_qc_iou_v4'
...
for idx, (key_a, key_b) in enumerate(pairs):
    ...
    # Load slices on-demand — only 2 tiles in memory at a time
    print(f"  Loading {key_a}...", end=" ", flush=True)
    _, ref = load_boundary_slices(key_a, masks.get(key_a, np.ones((4200,4200), dtype=np.uint8)))
    print(f"done. Loading {key_b}...", end=" ", flush=True)
    mov, _ = load_boundary_slices(key_b, masks.get(key_b, np.ones((4200,4200), dtype=np.uint8)))
    print("done")
...
save_path = f'{BASE}/registration_video/auto_align_transforms_iou_v4.json'
