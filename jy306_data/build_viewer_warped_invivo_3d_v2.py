...
OUT = f'{BASE}/3d_viewer/viewer_warped_invivo_3d_v2.html'
...
ELASTIX_SAVE_DIR = f'{BASE}/png_exports/registration_per_tile_elastix/transforms_3d'
os.makedirs(ELASTIX_SAVE_DIR, exist_ok=True)

def get_elastix_params():
    """Return the elastix parameter map used for all registrations."""
...
def save_transform_params(tp, path):
    """Save elastix transform parameter map to text file."""
    sitk.WriteParameterFile(tp[0], path)

def load_transform_params(path):
    """Load elastix transform parameter map from text file."""
    tp = sitk.ReadParameterFile(path)
    return (tp,)

def apply_transform(moving_img, tp_tuple):
    """Apply a saved transform parameter map to an image."""
    transformix = sitk.TransformixImageFilter()
    transformix.SetMovingImage(sitk.GetImageFromArray(moving_img))
    transformix.SetTransformParameterMap(tp_tuple)
    transformix.SetLogToConsole(False)
    transformix.Execute()
    return sitk.GetArrayFromImage(transformix.GetResultImage())

def run_elastix_with_raw(fixed_np, moving_filt, moving_raw, save_path=None):
    """Run elastix on filtered images, apply same transform to raw image.
    Optionally save the transform parameter map to save_path."""
    ...
    elastix.SetParameterMap(get_elastix_params())
    try:
        elastix.Execute()
        tp = elastix.GetTransformParameterMap()
        if save_path:
            save_transform_params(tp, save_path)
        result = apply_transform(moving_raw, tp)
        return result, tp
    except Exception:
...
print("  Median-filter bg subtraction for elastix registration...")
...
for tile in sorted(tile_lm_files.keys()):
    ...
    # Save per-tile directory for transforms
    tile_save_dir = f'{ELASTIX_SAVE_DIR}/{tile}'
    os.makedirs(tile_save_dir, exist_ok=True)

    # Save affine transform
    np.savez(f'{tile_save_dir}/affine_3d.npz',
             A_phys=A, A_T=A_T,
             M_fwd=M_fwd, t_fwd=t_fwd, M_inv=M_inv, offset_inv=offset_inv,
             z_offset=z_offset,
             nd2_z_vals=np.array(nd2_z_vals),
             ev_nd2=ev_nd2, pcd_iv=pcd_iv,
             errors=errors)

    # Save elastix config
    pm = get_elastix_params()
    config_path = f'{tile_save_dir}/elastix_config.txt'
    sitk.WriteParameterFile(pm, config_path)

    # Warp each nd2 z-slice (affine + elastix)
    # Compute z_iv at EACH landmark position — tiles with strong z-xy coupling
    # can have tile center map wildly out of range even when landmarks are valid
    for z_nd2 in range(12):
        ...
        # Affine warp both filtered (for elastix) and raw (for display)
        ...
        iv_affine_raw = cv2.warpAffine(iv_vol_raw[z_iv], M2d, (nd2_w, nd2_h),
                                        flags=cv2.INTER_LINEAR | cv2.WARP_INVERSE_MAP, borderValue=0)
        # Elastix B-spline on top of affine — use cached transform if available
        tp_path = f'{tile_save_dir}/elastix_tp_z{z_nd2:02d}.txt'
        if os.path.exists(tp_path):
            print(f"    cached z_nd2={z_nd2} z_iv={z_iv}")
            tp = load_transform_params(tp_path)
            iv_warped = apply_transform(iv_affine_raw, tp)
        else:
            nd2_sl = nd2_slices[z_nd2].astype(np.float32)
            print(f"    elastix z_nd2={z_nd2} z_iv={z_iv} ...", end="", flush=True)
            iv_warped, tp = run_elastix_with_raw(nd2_sl, iv_affine_filt, iv_affine_raw, save_path=tp_path)
            print(" done")

        # Save 2D affine matrix and z_iv for this z-slice
        np.savez(f'{tile_save_dir}/slice_z{z_nd2:02d}.npz', M2d=M2d, z_iv=z_iv)
...
def get_warped_iv(tile, z_nd2, nd2_y=2100, nd2_x=2100):
    """Get warped in-vivo slice using saved affine + elastix transforms.
    ...
    iv_affine_raw = cv2.warpAffine(iv_vol_raw[z_iv], M2d, (4200, 4200),
                                    flags=cv2.INTER_LINEAR | cv2.WARP_INVERSE_MAP, borderValue=0)
    # Use saved elastix transform if available
    tp_path = f'{ELASTIX_SAVE_DIR}/{tile}/elastix_tp_z{z_nd2:02d}.txt'
    if os.path.exists(tp_path):
        tp = load_transform_params(tp_path)
        iv_w = apply_transform(iv_affine_raw, tp)
    else:
        iv_w = iv_affine_raw
