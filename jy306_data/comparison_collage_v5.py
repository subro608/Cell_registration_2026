...
def run_elastix(ref, mov_rigid, n_iter):
    ...
    el.SetFixedImage(sitk.GetImageFromArray(ref_ds))
    el.SetMovingImage(sitk.GetImageFromArray(mov_ds))
...
def build_pair_row(sheet_row_img):
    """Resize a pair row for the contact sheet."""
    target_h = 300
    scale = target_h / sheet_row_img.shape[0]
    target_w = int(sheet_row_img.shape[1] * scale)
    return cv2.resize(sheet_row_img, (target_w, target_h))
...
hdr_colors = [(220,220,220), (200,255,200), (200,255,255), (255,200,150), (255,230,100)]
...
for idx, (key_a, key_b) in enumerate(PAIRS):
    ...
    ref_raw = cv2.imread(f'{PNG_DIR}/{key_a}/GFP_z011.png', cv2.IMREAD_GRAYSCALE)
    mov_raw = cv2.imread(f'{PNG_DIR}/{key_b}/GFP_z000.png', cv2.IMREAD_GRAYSCALE)
    if ref_raw is None or mov_raw is None:
        print(f"  SKIP: image not found"); continue
    ref_raw = ref_raw.astype(np.float32)
    mov_raw = mov_raw.astype(np.float32)
    h, w = ref_raw.shape
    ...
    # Farneback
    print("  Farneback...", end=' ', flush=True)
    ...
    print(f"NCC={ncc_fb:.4f}")

    # Elastix 500
    print("  Elastix 500...", end=' ', flush=True)
    ...
    print(f"NCC={ncc_e500:.4f}")

    # Elastix 2000
    print("  Elastix 2000...", end=' ', flush=True)
    ...
    for mi, (mname, col) in enumerate(zip(METHODS, hdr_colors)):
    ...
    # Save individual pair PNG
    cv2.imwrite(f'{OUT_DIR}/{pair_key}.png', sheet)
    print(f"  Saved: {OUT_DIR}/{pair_key}.png")

    # Add to contact sheet (downscaled row)
    ...
    all_results[pair_key] = {
        'ncc_rigid': ncc_rigid, 'ncc_fb': ncc_fb,
        'ncc_e500': ncc_e500,   'ncc_e2000': ncc_e2000
    }

# ---- High-res contact sheet ----
print("\nBuilding high-res contact sheet...")
contact_sheet = np.vstack(contact_rows)
contact_path = f'{OUT_DIR}/ALL_PAIRS_contact_sheet.png'
cv2.imwrite(contact_path, contact_sheet)
print(f"Contact sheet saved: {contact_path}  ({contact_sheet.shape[1]}x{contact_sheet.shape[0]}px)")

with open(f'{OUT_DIR}/results_v5.json', 'w') as f:
    json.dump(all_results, f, indent=2)

print("\n=== SUMMARY ===")
for k, v in all_results.items():
    print(f"  {k}: rigid={v['ncc_rigid']:.3f} fb={v['ncc_fb']:.3f} e500={v['ncc_e500']:.3f} e2000={v['ncc_e2000']:.3f}")
