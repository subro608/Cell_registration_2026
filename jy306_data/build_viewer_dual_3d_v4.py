...
OUT = f"{BASE}/3d_viewer/viewer_dual_3d_v4.html"
...
PHYS_RADIUS_UM = 50
PHYS_Z_HALF_UM = 6
CROP_ND2 = int(round(PHYS_RADIUS_UM / 0.645))   # 78 px
CROP_JY  = int(round(PHYS_RADIUS_UM / 0.6835))  # 73 px
DZ_ND2   = int(np.ceil(PHYS_Z_HALF_UM / 2.0))   # ±3 slices
DZ_JY    = int(np.ceil(PHYS_Z_HALF_UM / 3.0))   # ±2 slices
VOXEL_THRESH_EX = 12
VOXEL_THRESH_IV = 25

# GP parameters
GP_LENGTHSCALE = 1.0
GP_INTERP = 1     # no GP — keep HTML under 100MB
GP_NOISE = 0.01
...
# ============================================================
# 1. Load nd2 GFP PNGs — stack all tiles in z (no masks)
# ============================================================
print("Loading nd2 GFP tiles (raw, no alignment)...")
...
# Load all slices, downsample, store
...
for ti, tile in enumerate(TILE_ORDER):
    ...
    for zi in range(SLICES_PER_TILE):
        ...
        ex_vol[global_z] = img[::DS_EX, ::DS_EX][:ex_ny, :ex_nx]
    if ti % 5 == 0:
        print(f"  tile {ti+1}/{n_tiles} ({tile})")
print(f"  Loaded all {n_tiles} tiles")
...
<title>Ex-vivo (nd2 raw) + In-vivo 3D v4 — Matched Cells</title>
<style>
...
<div id="info">
  <b>Ex-vivo (nd2 raw stack) + In-vivo (JY306 s80)</b> — v4<br>
  {N_CELLS} matched cells | nd2 native space (no IOU/elastix)<br>
  ...
  <table style="font-size:10px;border-collapse:collapse;width:100%">
    ...
    <tr><td>Alignment</td><td colspan="2">None (raw nd2 stack, landmarks in native space)</td></tr>
  </table>
</div>
<div id="controls">
  <span class="section-title">Ex-vivo (nd2 raw)</span>
