...
out_dir = f'{BASE}/png_exports/z_stitch_comparison_v4'
os.makedirs(out_dir, exist_ok=True)

for group_name, group_pairs in ROW_GROUPS.items():
    # skip groups with no rows 2-4 pairs
    if not any(a.split('_')[0] in ROWS_CHANGED or b.split('_')[0] in ROWS_CHANGED
               for a, b in group_pairs):
        print(f"Skipping {group_name} (not in rows 2-4)")
        continue
