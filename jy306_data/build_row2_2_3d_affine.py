#!/usr/bin/env python3
"""
Row2_2 tile: 3D affine registration (in-vivo JY306 → nd2 tile).
For each of 16 in-vivo z-slices: use 3D affine to find corresponding nd2 z,
warp XY via 2D slice extracted from 3D inverse, show contact sheet.
"""
import numpy as np
import cv2
import os
...
print(f"  iv z: {sorted(set(pcd_iv[:, 0].astype(int)))}")
print(f"  nd2 z: {sorted(set(nd2_z_vals))}")

# ============================================================
# 3D affine: in-vivo (x,y,z) µm → nd2 (x,y,z) µm
# ============================================================
print("\nFitting 3D affine...")
...
print(f"  3D affine: mean={errors.mean():.1f}µm median={np.median(errors):.1f}µm max={errors.max():.1f}µm")
print(f"  Scales: {S[0]:.3f}, {S[1]:.3f}, {S[2]:.3f}")

# ============================================================
# Build pixel-space forward/inverse
# A maps: iv_um(x,y,z) → nd2_um(x,y,z)
# We need pixel-space: iv_px(z,y,x) → nd2_px(z,y,x)
# ============================================================
sx, sy, sz = IV_XY_UM, IV_XY_UM, IV_Z_UM
ex, ey, ez = ND2_XY_UM, ND2_XY_UM, ND2_Z_UM

# Forward: iv_px(z,y,x) → nd2_um(x,y,z) → nd2_px(z,y,x)
# iv_um = (iv_px_x * sx, iv_px_y * sy, iv_px_z * sz)
# nd2_um = A @ [iv_um; 1]
# nd2_px = nd2_um / (ex, ey, ez)

# M_fwd maps (z,y,x)_iv_px → (z,y,x)_nd2_px
M_fwd = np.array([
...
# ============================================================
# For each of 16 in-vivo z: find nd2 z via affine, warp XY
# ============================================================
print("\nMapping in-vivo slices...")
...
# Predicted landmarks in nd2 px
nd2_lm_actual = ev_nd2[:, :2]  # (N, 2) x, y in nd2 px
nd2_lm_pred = predicted[:, :2] / ND2_XY_UM  # predicted µm → nd2 px
iv_lm_z = pcd_iv[:, 0]  # z in JY306 slices

for z_iv in range(nz_iv):
    # Find corresponding nd2 z via forward transform
    ...
    print(f"  iv_z={z_iv:2d} → nd2_z={z_nd2:2d} (float={z_nd2_float:.1f})")

    # 2D warp: extract from 3D inverse at fixed z_nd2
    # For each (y_nd2, x_nd2) in nd2 space, find (y_iv, x_iv) in iv space
    # inv maps (z,y,x)_nd2_px → (z,y,x)_iv_px
    M2d = np.array([
    ...
    ov_rgb[:, :, 0] = norm8(iv_d)   # magenta = in-vivo
    ov_rgb[:, :, 2] = norm8(iv_d)

    # Draw landmarks near this in-vivo z
    n_lm = 0
    for i in range(N_LM):
...
cv2.putText(sheet,
            f'{TILE}: 3D affine ({N_LM} lm) | mean={errors.mean():.1f}um | scales={S[0]:.2f},{S[1]:.2f},{S[2]:.2f}',
...
out_path = f'{OUT_DIR}/registration_{TILE}_3d_affine.png'
cv2.imwrite(out_path, sheet)
print(f"\nSaved: {out_path}")
print(f"Sheet size: {sheet.shape[1]}x{sheet.shape[0]}")

# Save affine
np.savez(f'{BASE}/registration_video/affine_3d_{TILE}.npz',
         affine_3x4=A, src_um=src, dst_um=dst,
         errors=errors, predicted=predicted)
print(f"Affine saved: affine_3d_{TILE}.npz")
