...
OUT = f'{BASE}/3d_viewer/viewer_warped_invivo_3d_v3_elastix.html'
...
for tile in sorted(tile_lm_files.keys()):
    ...
    # Load elastix transforms for this tile
    tfm_dir = f'{BASE}/png_exports/registration_per_tile_elastix/{tile}/transforms'
    has_elastix = os.path.isdir(tfm_dir)
    if has_elastix:
        print(f"    Elastix transforms dir found")
    ...
    for z_nd2 in range(12):
        ...
        # Check for saved per-slice elastix transform
        ...
        # Apply elastix on top of affine
        if os.path.exists(tp_path):
            ...
            try:
                tx.Execute()
                iv_warped = sitk.GetArrayFromImage(tx.GetResultImage()).astype(np.float32)
                print(f"    affine+elastix z_nd2={z_nd2} z_iv={z_iv}")
            except Exception:
                iv_warped = iv_affine_raw
                print(f"    affine-only z_nd2={z_nd2} z_iv={z_iv} (elastix failed)")
        else:
            iv_warped = iv_affine_raw
            print(f"    affine-only z_nd2={z_nd2} z_iv={z_iv} (no elastix)")
    ...
    for i in range(N_LM):
        ...
        # Blob-distance filter: warp Gaussian blob through affine+elastix, measure centroid
        z_iv_i = int(round(pcd_iv[i, 0]))
        z_iv_i = max(0, min(nz_iv - 1, z_iv_i))

        # Load per-slice affine if available
        slice_path_i = f'{tfm_dir}/slice_ziv{z_iv_i:02d}_znd2{z_nd2_i:02d}.npz'
        tp_path_i = f'{tfm_dir}/elastix_tp_ziv{z_iv_i:02d}_znd2{z_nd2_i:02d}.txt'
        if os.path.exists(slice_path_i):
            sd_i = np.load(slice_path_i)
            M2d_i = sd_i['M2d']
        else:
            M2d_i = np.array([
        ...
        blob_affine = cv2.warpAffine(bimg, M2d_i, (nd2_w, nd2_h),
                                      flags=cv2.INTER_LINEAR | cv2.WARP_INVERSE_MAP, borderValue=0)
        # Warp through elastix
        if os.path.exists(tp_path_i):
            tp_i = sitk.ReadParameterFile(tp_path_i)
            tx_i = sitk.TransformixImageFilter()
            tx_i.SetTransformParameterMap((tp_i,))
            tx_i.SetLogToConsole(False)
            tx_i.SetMovingImage(sitk.GetImageFromArray(blob_affine))
            try:
                tx_i.Execute()
                blob_warped = sitk.GetArrayFromImage(tx_i.GetResultImage()).astype(np.float32)
            except Exception:
...
warped_cache = {}
ELASTIX_TRANSFORMS_BASE = f'{BASE}/png_exports/registration_per_tile_elastix'

def get_warped_iv(tile, z_nd2, nd2_y=2100, nd2_x=2100):
    """Get warped in-vivo slice using 3D affine + elastix.
    ...
    etfm_dir = f'{ELASTIX_TRANSFORMS_BASE}/{tile}/transforms'
    slice_path = f'{etfm_dir}/slice_ziv{z_iv:02d}_znd2{z_nd2:02d}.npz'
    tp_path = f'{etfm_dir}/elastix_tp_ziv{z_iv:02d}_znd2{z_nd2:02d}.txt'
    ...
    if os.path.exists(tp_path):
        ...
        try:
            tx.Execute()
            iv_w = sitk.GetArrayFromImage(tx.GetResultImage()).astype(np.float32)
        except Exception:
            iv_w = iv_affine_raw
    else:
        iv_w = iv_affine_raw
...
<title>In-vivo Warped (Elastix) into Ex-vivo Stitched — 3D + Patches</title>
<style>
