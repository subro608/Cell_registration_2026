...
OUT_HTML = f'{OUT_DIR}/landmark_patches.html'
...
for tile in sorted(tile_lm_files.keys()):
    # Try TPS transform first, fall back to elastix
    tps_path = f'{BASE}/png_exports/registration_per_tile_tps/{tile}/tps_transform_{tile}.npz'
    use_tps = os.path.exists(tps_path)
    tps_data = None
    if use_tps:
        td_npz = np.load(tps_path)
        tps_data = {
            'W': td_npz['W'], 'a': td_npz['a_coeff'], 'src': td_npz['src_pts'],
            'W_inv': td_npz['W_inv'], 'a_inv': td_npz['a_inv'], 'src_inv': td_npz['src_inv'],
        }
        print(f"  Using TPS transform ({len(tps_data['src'])} control pts)")
    tfm_dir = f'{TRANSFORMS_BASE}/{tile}/transforms'
    if not use_tps and (not os.path.isdir(tfm_dir) or not glob.glob(f'{tfm_dir}/elastix_tp_*.txt')):
    ...
    # Filter out bad landmarks: edge z-matches + high affine error + z-mapping outliers
    Z_EDGE_MARGIN = 0.5
    AFF_ERR_MAX_UM = 10.0
    n_z_max = len(nd2_slices) - 1  # 11 for 12-slice volume

    # Pass 1: edge z and affine error
    pass1_good = set()
    skipped_edge = 0
    skipped_err = 0
    for i in range(N_LM):
        g = nd2_z_vals[i]
        if g < Z_EDGE_MARGIN or g > n_z_max - Z_EDGE_MARGIN:
            skipped_edge += 1
        elif errors_xy_um[i] > AFF_ERR_MAX_UM:
            skipped_err += 1
        else:
            pass1_good.add(i)

    # Pass 2: z-mapping consistency — fit z_nd2 = a*z_iv + b, remove outliers
    skipped_z = 0
    good_lm = set()
    if len(pass1_good) >= 4:
        good_list = sorted(pass1_good)
        z_iv_arr = np.array([pcd_iv[i, 0] for i in good_list])
        z_nd2_arr = np.array([nd2_z_vals[i] for i in good_list])
        A_z = np.column_stack([z_iv_arr, np.ones(len(good_list))])
        coeff_z, _, _, _ = np.linalg.lstsq(A_z, z_nd2_arr, rcond=None)
        pred_z_local = A_z @ coeff_z
        z_resid = np.abs(z_nd2_arr - pred_z_local)
        med_r = np.median(z_resid)
        mad_r = np.median(np.abs(z_resid - med_r))
        z_thresh = med_r + 3 * max(mad_r, 0.3)
        for j, i in enumerate(good_list):
            if z_resid[j] > z_thresh:
                skipped_z += 1
            else:
                good_lm.add(i)
    else:
    ...
    n_filt = skipped_edge + skipped_err + skipped_z
    if n_filt > 0:
        parts = []
        if skipped_edge: parts.append(f"{skipped_edge} edge z-match")
        if skipped_err: parts.append(f"{skipped_err} aff>{AFF_ERR_MAX_UM}µm")
        if skipped_z: parts.append(f"{skipped_z} z-outlier")
        print(f"  Filtered {n_filt}/{N_LM} landmarks ({', '.join(parts)})")
    ...
    for (z_iv, z_nd2), lm_indices in sorted(z_pair_to_lm.items()):
        ...
        for i in lm_indices:
            ...
            if i in warped_binaries:
                blob_patch = crop_patch(warped_binaries[i], cx, cy, PATCH_R)
                bmax = blob_patch.max()
                if bmax > 1:
...
<html>
<head>
<title>Landmark Patches — Ex-vivo vs In-vivo (TPS+IRLS)</title>
<style>
...
<body>
<h1>Landmark Patches: Ex-vivo vs In-vivo (TPS+IRLS corrected)</h1>
<div class="controls">
  <label>Tile:</label>
