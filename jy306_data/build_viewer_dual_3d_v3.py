...
OUT = f"{BASE}/3d_viewer/viewer_dual_3d_v3.html"

DS_EX = 3
DS_IV = 1
NORM = 4000
PATCH_SZ = 80
# Same ±2 slices, same 100µm XY FOV for both
DZ_SLICES = 2
PHYS_RADIUS = 50       # 50µm half-width → 100µm FOV for both
CROP_ND2 = int(round(PHYS_RADIUS / 0.645))    # 78 px
CROP_JY  = int(round(PHYS_RADIUS / 0.6835))   # 73 px
DZ_ND2 = DZ_SLICES
DZ_JY  = DZ_SLICES
VOXEL_THRESH_EX = 8
VOXEL_THRESH_IV = 50
...
# Normalize coordinates to [0, 1] based on voxel bounding box
ex_vx_raw = exx.astype(np.float32)
ex_vy_raw = ey.astype(np.float32)
ex_vz_raw = np.array([target_zs[z] for z in ez], dtype=np.float32)
# Bounding box of ex-vivo voxels (in grid units)
ex_bbox = dict(x0=ex_vx_raw.min(), x1=ex_vx_raw.max(),
               y0=ex_vy_raw.min(), y1=ex_vy_raw.max(),
               z0=ex_vz_raw.min(), z1=ex_vz_raw.max())
ex_span = max(ex_bbox['x1']-ex_bbox['x0'], ex_bbox['y1']-ex_bbox['y0'], ex_bbox['z1']-ex_bbox['z0'])
ex_vx = (ex_vx_raw - ex_bbox['x0']) / ex_span
ex_vy = (ex_vy_raw - ex_bbox['y0']) / ex_span
ex_vz = (ex_vz_raw - ex_bbox['z0']) / ex_span
ex_vv = ex_vals.astype(np.float32) / 255.0
print(f"  Ex-vivo bbox: x[{ex_bbox['x0']:.0f}-{ex_bbox['x1']:.0f}] y[{ex_bbox['y0']:.0f}-{ex_bbox['y1']:.0f}] z[{ex_bbox['z0']:.1f}-{ex_bbox['z1']:.1f}], span={ex_span:.0f}")
del gp_vol, gp_u8
...
# Normalize coordinates to [0, 1] based on voxel bounding box
iv_vx_raw = ixx.astype(np.float32)
iv_vy_raw = iyy.astype(np.float32)
iv_vz_raw = izz.astype(np.float32)
iv_bbox = dict(x0=iv_vx_raw.min(), x1=iv_vx_raw.max(),
               y0=iv_vy_raw.min(), y1=iv_vy_raw.max(),
               z0=iv_vz_raw.min(), z1=iv_vz_raw.max())
iv_span = max(iv_bbox['x1']-iv_bbox['x0'], iv_bbox['y1']-iv_bbox['y0'], iv_bbox['z1']-iv_bbox['z0'])
iv_vx = (iv_vx_raw - iv_bbox['x0']) / iv_span
iv_vy = (iv_vy_raw - iv_bbox['y0']) / iv_span
iv_vz = (iv_vz_raw - iv_bbox['z0']) / iv_span
iv_vv = iv_vals.astype(np.float32) / 255.0
print(f"  In-vivo bbox: x[{iv_bbox['x0']:.0f}-{iv_bbox['x1']:.0f}] y[{iv_bbox['y0']:.0f}-{iv_bbox['y1']:.0f}] z[{iv_bbox['z0']:.0f}-{iv_bbox['z1']:.0f}], span={iv_span:.0f}")
del iv_vol, iv_u8
...
def depth_color(t):
    """Map depth fraction t (0=shallow, 1=deep) to RGB: green -> yellow -> red."""
    r = min(2 * t, 1.0)
    g = min(2 * (1 - t), 1.0)
    return (r, g, 0.0)

def make_depth_mip(slices_list, patch_sz):
    """Create depth-coded MIP from list of (slice, depth_fraction) tuples."""
    if not slices_list:
        return np.zeros((patch_sz, patch_sz, 3), dtype=np.uint8)
    # Stack and find argmax
    stack = np.array([s for s, _ in slices_list])  # (nz, h, w)
    depths = np.array([d for _, d in slices_list])  # (nz,)
    # Normalize stack
    p99 = np.percentile(stack[stack > 0], 99) if (stack > 0).any() else 1
    stack_norm = np.clip(stack / max(p99, 1), 0, 1)
    # Argmax MIP — which slice is brightest at each pixel
    argmax_z = np.argmax(stack_norm, axis=0)  # (h, w)
    mip_val = np.max(stack_norm, axis=0)       # (h, w) intensity
    # Build color image
    h, w = argmax_z.shape
    ...
    cv2.drawMarker(resized, (patch_sz//2, patch_sz//2), (255, 255, 255), cv2.MARKER_CROSS, 20, 1)
    return resized

# Absolute depth colormap: 0µm (green) → TOTAL_DEPTH µm (red)
# Both ex-vivo and in-vivo share this axis.
# Ex-vivo: absolute z from stitched coords (1µm iso), nd2 slices at 2µm spacing
# In-vivo: offset to matched ex-vivo z, then ±DZ slices at 3µm spacing
TOTAL_DEPTH = float(ex_nz_full)  # 516µm
ND2_Z_UM = 2.0   # nd2 z-step in µm
S80_Z_UM = 3.0   # s80 z-step in µm
print(f"  Absolute depth axis: 0 → {TOTAL_DEPTH:.0f} µm")

# 4 columns: ex-vivo, ex-depth, iv, iv-depth
patch_strip_w = PATCH_SZ * 4
patch_strip_h = PATCH_SZ * N_CELLS
patch_strip = np.zeros((patch_strip_h, patch_strip_w, 3), dtype=np.uint8)

for i in range(N_CELLS):
    tile = all_tiles[i]
    col_c = int(round(ev_nd2[i, 0]))
    row_c = int(round(ev_nd2[i, 1]))
    z_in_tile = int(cell_nd2_z[i])
    # Absolute depth of this cell's center in stitched 1µm iso space
    cell_abs_z_um = float(st_pts[i, 0])  # stitched z in µm (1µm iso)

    # Ex-vivo: collect slices with ABSOLUTE depth fractions
    slices = []
    slices_depth = []
    for dz in range(-DZ_ND2, DZ_ND2 + 1):
        ...
        if 0 <= zz < SLICES_PER_TILE and (tile, zz) in nd2_pages:
            ...
            slices.append(crop)
            abs_z = cell_abs_z_um + dz * ND2_Z_UM
            t = np.clip(abs_z / TOTAL_DEPTH, 0, 1)
            slices_depth.append((crop, t))
    ...
    # Depth-coded MIP (absolute)
    depth_rgb = make_depth_mip(slices_depth, PATCH_SZ)

    row = i * PATCH_SZ
    patch_strip[row:row+PATCH_SZ, 0:PATCH_SZ] = mip_rgb
    patch_strip[row:row+PATCH_SZ, PATCH_SZ:PATCH_SZ*2] = depth_rgb

    # In-vivo: offset to matched ex-vivo z, then ±DZ at 3µm spacing
    ...
    for dz in range(-DZ_JY, DZ_JY + 1):
        ...
        if 0 <= zz < iv_nz_nat:
            ...
            crop = page[y0:y1, x0:x1].astype(np.float32)
            slices_iv.append(crop)
            # Absolute depth: ex-vivo z of this cell + in-vivo dz offset in µm
            abs_z = cell_abs_z_um + dz * S80_Z_UM
            t = np.clip(abs_z / TOTAL_DEPTH, 0, 1)
            slices_iv_depth.append((crop, t))
    ...
    depth_iv_rgb = make_depth_mip(slices_iv_depth, PATCH_SZ)

    patch_strip[row:row+PATCH_SZ, PATCH_SZ*2:PATCH_SZ*3] = mip_iv_rgb
    patch_strip[row:row+PATCH_SZ, PATCH_SZ*3:PATCH_SZ*4] = depth_iv_rgb
...
cell_info_js = []
for i in range(N_CELLS):
    # Ex-vivo: stitched coords (z,y,x) in 1µm iso -> convert to DS3 grid -> bbox normalize
    ex_x_grid = st_pts[i, 2] / DS_EX   # to DS3 grid
    ex_y_grid = st_pts[i, 1] / DS_EX
    ex_z_grid = st_pts[i, 0]            # z is in original 1µm iso
    # Map through GP z -> find closest target_z index, then to bbox-normalized
    # Approximate: z in 1µm iso / (ex_nz/nz_out * nz_out) ~ z / ex_nz * nz
    ex_x_n = (ex_x_grid - ex_bbox['x0']) / ex_span
    ex_y_n = (ex_y_grid - ex_bbox['y0']) / ex_span
    ex_z_n = (ex_z_grid - ex_bbox['z0']) / ex_span
    # IV: native s80 pixels -> 1µm iso grid -> bbox normalize
    iv_x_grid = iv_pts[i, 2] * IV_XY_UM / DS_IV  # native px -> µm -> DS grid
    iv_y_grid = iv_pts[i, 1] * IV_XY_UM / DS_IV
    iv_z_grid = iv_pts[i, 0] * IV_Z_UM / DS_IV   # native z -> µm -> DS grid
    iv_x_n = (iv_x_grid - iv_bbox['x0']) / iv_span
    iv_y_n = (iv_y_grid - iv_bbox['y0']) / iv_span
    iv_z_n = (iv_z_grid - iv_bbox['z0']) / iv_span
...
# Physical sizes for info display
ex_x_um = ex_w
ex_y_um = ex_h
ex_z_um = ex_nz_full
# Both volumes are bbox-normalized to [0, max_span] then /max_span -> [0,1]
# So JS just needs sx=sy=sz=1.0 for both — bounding boxes already match
ex_sx = ex_sy = ex_sz = 1.0
iv_sx = iv_sy = iv_sz = 1.0
...
<title>Ex-vivo + In-vivo 3D v3 — Matched Cells</title>
<style>
  ...
  #patchPanel.show {{ height:200px; }}
  #patchInner {{ display:flex; align-items:center; justify-content:center; gap:12px; height:100%; }}
  #patchPanel canvas {{ width:110px; height:110px; image-rendering:pixelated; }}
  .plabel {{ color:#0f0; font-size:11px; text-align:center; margin-bottom:3px; }}
  .plabel-iv {{ color:#f0f; font-size:11px; text-align:center; margin-bottom:3px; }}
  .plabel-depth {{ color:#fc0; font-size:11px; text-align:center; margin-bottom:3px; }}
  .ppair {{ text-align:center; }}
  .ppair canvas {{ border:2px solid #0f0; }}
  .ppair-iv canvas {{ border:2px solid #f0f; }}
  .ppair-depth canvas {{ border:2px solid #fc0; }}
  #depthLegend {{ display:inline-block; width:80px; height:12px; border-radius:3px;
...
<div id="info">
  <b>Ex-vivo (stitched v5) + In-vivo (JY306 s80)</b> — v3<br>
  {N_CELLS} matched cells | ±{DZ_SLICES} slices, {PHYS_RADIUS*2:.0f}µm FOV<br>
...
<div id="controls">
  ...
  <label>Opacity: <input type="range" id="ivOpac" min="1" max="100" value="74" style="width:90px"><span id="ivOpVal">74</span></label>
...
<div id="patchPanel">
  ...
  <div id="patchInner">
    <div class="ppair">
      <div class="plabel">Ex-vivo MIP</div>
      <canvas id="patchExCv" width="{PATCH_SZ}" height="{PATCH_SZ}"></canvas>
    </div>
    <div class="ppair ppair-depth">
      <div class="plabel-depth">Ex-vivo Depth</div>
      <canvas id="patchExDepthCv" width="{PATCH_SZ}" height="{PATCH_SZ}"></canvas>
    </div>
    <div style="text-align:center;min-width:100px" id="pairInfo">
      <span style="color:#0f0;font-size:14px">&#8596;</span>
      <div id="depthLegend"></div>
      <div class="depth-labels"><span>0&micro;m</span><span>{int(TOTAL_DEPTH)}&micro;m</span></div>
    </div>
    <div class="ppair ppair-iv">
      <div class="plabel-iv">In-vivo MIP</div>
      <canvas id="patchIvCv" width="{PATCH_SZ}" height="{PATCH_SZ}"></canvas>
    </div>
    <div class="ppair ppair-depth">
      <div class="plabel-depth">In-vivo Depth</div>
      <canvas id="patchIvDepthCv" width="{PATCH_SZ}" height="{PATCH_SZ}"></canvas>
    </div>
  </div>
</div>
...
function showPatch(idx) {
  ...
  // Col 1: ex-vivo depth MIP
  const exDCv=document.getElementById('patchExDepthCv'),exDCtx=exDCv.getContext('2d');
  exDCtx.clearRect(0,0,PATCH_SZ,PATCH_SZ);
  exDCtx.drawImage(patchStripImg,PATCH_SZ,sy,PATCH_SZ,PATCH_SZ,0,0,PATCH_SZ,PATCH_SZ);
  // Col 2: in-vivo grayscale MIP
  const ivCv=document.getElementById('patchIvCv'),ivCtx=ivCv.getContext('2d');
  ivCtx.clearRect(0,0,PATCH_SZ,PATCH_SZ);
  ivCtx.drawImage(patchStripImg,PATCH_SZ*2,sy,PATCH_SZ,PATCH_SZ,0,0,PATCH_SZ,PATCH_SZ);
  // Col 3: in-vivo depth MIP
  const ivDCv=document.getElementById('patchIvDepthCv'),ivDCtx=ivDCv.getContext('2d');
  ivDCtx.clearRect(0,0,PATCH_SZ,PATCH_SZ);
  ivDCtx.drawImage(patchStripImg,PATCH_SZ*3,sy,PATCH_SZ,PATCH_SZ,0,0,PATCH_SZ,PATCH_SZ);
  // Info text
  const ci=cellInfo[idx];
  document.getElementById('pairInfo').innerHTML='<b>#'+idx+'</b> ('+tileNames[idx]+')<br>'+
    '<span style="color:#0f0;font-size:10px">MIP z'+ci[1]+'-'+ci[2]+' (±'+DZ_ND2+'×2µm)</span><br>'+
    '<span style="color:#f0f;font-size:10px">MIP z'+ci[4]+'-'+ci[5]+' (±'+DZ_JY+'×3µm)</span><br>'+
    '<span style="color:#aaa;font-size:10px">FOV: '+FOV+'µm</span>'+
    '<div id="depthLegend"></div><div class="depth-labels"><span>0&micro;m</span><span>{int(TOTAL_DEPTH)}&micro;m</span></div>';
