...
OUT_DIR = f'{BASE}/png_exports/z_stitch_comparison_v5b'
...
METHODS  = ['Raw + Annotation', 'IOU Rigid', 'Elastix 500iter']
HDR_COLS = [(220,220,220), (200,255,200), (255,230,100)]
...
def run_elastix(ref, mov_rigid, n_iter):
    ...
    return cv2.resize(sitk.GetArrayFromImage(el.GetResultImage()).astype(np.float32), (w,h), interpolation=cv2.INTER_LINEAR)
...
for idx, (key_a, key_b) in enumerate(PAIRS):
    ...
    all_results[pair_key] = {'ncc_rigid': ncc_rigid, 'ncc_e500': ncc_e500}
...
with open(f'{OUT_DIR}/results_v5b.json', 'w') as f:
